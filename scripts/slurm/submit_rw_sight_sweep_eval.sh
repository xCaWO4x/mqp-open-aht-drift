#!/bin/bash
# Submit the Q3_rw sight-sweep greedy evals (sight in {4,5,6,7}).
# Prerequisite: the matching training jobs have finished and written
# results/q3_rw_sight{K}_stationary/checkpoints/gpl_final.pt.
#
# Usage: bash scripts/slurm/submit_rw_sight_sweep_eval.sh
...
for K in "${SIGHTS[@]}"; do
  CKPT="results/q3_rw_sight${K}_stationary/checkpoints/gpl_final.pt"
  if [[ ! -f "${CKPT}" ]]; then
    echo "WARNING: checkpoint missing for sight=${K}: ${CKPT} — skipping"
    continue
  fi
  echo "Submitting Q3_rw greedy eval (sight=${K})..."
  sbatch --export=ALL "scripts/slurm/q3_rw_sight${K}_greedy_eval.slurm"
done
